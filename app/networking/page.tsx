...
function ConnectCard({ connect, onUpdateStatus }: { connect: Connect; onUpdateStatus: (c: Connect, status: string) => void }) {
  return (
    <div className="rounded-xl border bg-card border-border shadow-sm p-3 space-y-2">
        ...
        {STATUSES.map((s) => (
          <button
            ...
            className={`text-[10px] px-2 py-1 rounded-full font-medium transition-colors ${
              connect.status === s ? statusColors[s] : "border border-border text-muted-foreground hover:bg-muted"
            }`}
          >
            {STATUS_LABELS[s]}
          </button>
    ...
    <div className="max-w-lg lg:max-w-5xl mx-auto px-4 py-6 space-y-5">
      <div>
        ...
        <h1 className="text-xl font-bold flex items-center gap-2">
          <Users size={20} style={{ color: "var(--primary)" }} /> Networking
        </h1>
      </div>
      ...
      <div className="rounded-2xl p-4 border bg-card border-border shadow-sm space-y-3">
        <p className="text-sm font-semibold flex items-center gap-1.5">
          <Sparkles size={14} style={{ color: "var(--primary)" }} /> Capture from post
        </p>
        ...
        {captureMsg && <p className="text-xs" style={{ color: "var(--primary)" }}>{captureMsg}</p>}
      </div>
