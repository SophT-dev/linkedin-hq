...
| **LeadMagnets** | Lead magnet build pipeline. **⚠️ Read by HEADER NAME, not column position (as of 2026-07-11) — so columns can be freely reordered in the Sheet without breaking anything.** `lib/sheets.ts` (`readLeadMagnetSheet` → `loadLeadMagnets`/`createLeadMagnetRow`/`updateLeadMagnetRow`) and `scripts/capture-item.mjs` both resolve fields via a header→index map; only `readSheet` returns raw positional arrays. **34 columns as of Stage 16 (2026-07-16):** **`source_person`** (moved to column A 2026-07-11, it's the "Source Creator" — for our own magnets it's the profile they were published on, e.g. `Taha Anwar`; for received ones it's the external author) `\| id \| post_id \| slug \| status \| title \| hero_text \| value_props \| cta_text \| outline_md \| body_md \| notion_url \| landing_url \| gif_url \| created_at \| clicks \| conversions \| kind` (own/received) `\| source_post_url \| key_takeaway \| used_in_post \| domain \| post_likes \| post_comments \| cta_keyword \| contents_desc \| content_style \| format_tag \| visual_type \| lm_form \| lm_type \| vault_path \| starred \| source_person_url` — the last 12 are Stage 16's vault-intake fields (see below). A received lead magnet is a row with `kind=received`, logged via `node scripts/capture-item.mjs --type lead_magnet ...`. Status values (own): `researching \| outline_ready \| body_ready \| published \| error`. Status values (received): `unreviewed \| reviewed \| queued` (queued = the `/intake` form, drained by `/lm-intake`). **If you reorder/rename columns, update `scripts/format-all-tabs.mjs`'s `LeadMagnets.widths`/`statusCol` to match.** |
| **Template Library** | Curated top ~150-200 posts from the tagged corpus (`campaign-master/knowledge-base/learning-center/_tagged.jsonl` joined to each expert's `posts.json`), ranked by likes and separately by comments. This is the swipe/hook source for drafting — read it before writing a post, don't guess a hook. Columns: `hook \| suggested_format \| expert \| domain \| likes \| comments \| shares \| comment_to_like_ratio \| engagement_tier \| url \| date_added \| starred`. `comment_to_like_ratio` is a live Sheets formula (`=IF(likes=0,0,comments/likes)`) — added 2026-07-08 so real engagement (people compelled to respond) is visible separately from vanity likes; recalculates if you hand-edit a row. **`starred` (col L, added Stage 16)** — set by the extension's ⭐ star button or `add-template-library-entry.mjs --starred` for a favorite-only capture; rebuild-safe (`build-template-library.mjs`'s HEADERS includes it, so a rebuild never drops hand-starred rows' column). **No impressions column** — LinkedIn doesn't expose view counts for posts you don't own, and Apify can't get them either; only likes/comments/shares are public data. (Our *own* posts' views, once published, are a different, real data source — see the Posts tab below.) Rebuild with `node scripts/build-template-library.mjs [--top=100]` (idempotent — clears + rewrites data rows, preserving hand-edited `A2:L`). |
...
## Lead Magnet Vault (Stage 16, 2026-07-16)
Every lead magnet Sophiya earns now gets a real home instead of getting lost in DMs. Three intake
paths, one destination: **(a)** the `/lm-intake` skill (`.claude/skills/lm-intake/SKILL.md`) — batch
capture, 5-10 items at once; **(b)** the app's `/intake` form — a quick single-item capture that
writes a `status=queued` LeadMagnets row for `/lm-intake` to drain later (`--update-row`, never
re-appended); **(c)** the extension's ⭐ star button — favorite-only, no resource to archive, just a
Template Library row. Every FULL-CAPTURE item gets a vault folder at
`content/lead-magnets/received/<slug>/notes.md` (+ `assets/` for source-post visuals, `resource/` for
the archived magnet itself) **and** a LeadMagnets Sheet row (12 new columns as of this stage — see the
LeadMagnets tab row above).

**HARD RULE — a link is not a capture.** Expert-hosted resources (Notion pages, git repos, PDFs,
Gamma decks) die within weeks, so we always archive our own copy into the item's `resource/` folder.
`resource_archived: link-only` in a `notes.md` frontmatter is a temporary, unresolved state, never a
final one — `/lm-intake` greps for it and re-surfaces every pending link-only item at the top of
**every** run until it's actually archived.

**Row height:** appended LeadMagnets rows auto-grow — `/lm-intake`'s last phase always re-pins them
with `node scripts/format-all-tabs.mjs --tab LeadMagnets`. Run this manually too after any ad-hoc
append outside the skill.

**Legacy migration:** 49 old flat `content/lead-magnets/received/*.md` files predate the folder
shape and are **not** bulk-migrated — retrieval globs both `received/*.md` and `received/*/notes.md`,
and a flat file only gets moved into a proper `<slug>/notes.md` folder the next time something
actually touches it (lazy migration).
