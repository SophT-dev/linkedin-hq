...
| **LeadMagnets** | Lead magnet build pipeline (columns A-P, unchanged) PLUS 5 columns added in Stage 8 for lead magnets we *receive* (not build): `kind` (own/received) `\| source_person \| source_post_url \| key_takeaway \| used_in_post` (columns Q-U). The Vercel app only ever reads/writes A-P, so this is fully additive — a received lead magnet is just a row with `kind=received`, logged via `node scripts/capture-item.mjs --type lead_magnet ...`. Full columns: `id \| post_id \| slug \| status \| title \| hero_text \| value_props \| cta_text \| outline_md \| body_md \| notion_url \| landing_url \| gif_url \| created_at \| clicks \| conversions \| kind \| source_person \| source_post_url \| key_takeaway \| used_in_post`. Status values (own): `researching \| outline_ready \| body_ready \| published \| error`. Status values (received): `unreviewed \| reviewed`. |
