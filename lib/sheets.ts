...
const MY_COMMENTS_HEADER = ["commented_at", "post_url", "post_author", "comment_text"];

export async function saveMyComments(
  items: { url: string; author?: string; minutesAgo?: number; text?: string }[]
...
export async function loadMyComments(): Promise<{ urls: Set<string>; dates: string[] }> {
  let rows: string[][];
  try {
    rows = await readSheet(MY_COMMENTS_TAB, "A:D");
  } catch {
